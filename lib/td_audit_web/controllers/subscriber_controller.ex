defmodule TdAuditWeb.SubscriberController do
  ...
  def index(conn, _params) do
    with user <- conn.assigns[:current_resource],
         {:can, true} <- {:can, can?(user, list(Subscriber))},
  ...
  def create(conn, %{"subscriber" => subscriber_params}) do
    with user <- conn.assigns[:current_resource],
         {:can, true} <- {:can, can?(user, create(Subscriber))},
  ...
  def show(conn, %{"id" => id}) do
    with user <- conn.assigns[:current_resource],
         ...
         {:can, true} <- {:can, can?(user, view(subscriber))} do
  ...
  def delete(conn, %{"id" => id}) do
    with user <- conn.assigns[:current_resource],
         subscriber <- Subscribers.get_subscriber!(id),
         {:can, true} <- {:can, can?(user, delete(subscriber))},
